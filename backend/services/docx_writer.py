...
def _set_metadata(doc, result: ResearchResult, title: str):
    properties = doc.core_properties
    properties.title = f"{title} — {T.DOC_KIND}"
    properties.author = T.AUTHOR
    properties.subject = T.DOC_KIND
    properties.category = T.DOC_KIND
    properties.language = "en"
    properties.keywords = ", ".join(filter(None, [title, "research brief", result.provider]))
...
def _title_page(doc, result: ResearchResult, today: date):
    ...
    doc.add_paragraph(clean(result.title), style=TITLE_STYLE)
...
def build_docx(result: ResearchResult):
    ...
    _define_styles(doc)
    _setup_page(doc)
    _set_metadata(doc, result, title)
